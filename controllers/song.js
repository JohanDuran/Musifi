...
function getSongs(req,res) {
   ...
   if(!albumId){
    ...
    //find and populate primero se trae lo de album hacia song, luego dentro de album carga artist.
    find.populate({
       path:'album',
       populate:{
           path:'artist',
...
function uploadFile(req,res) {
    ...
    if(req.files){
        ...
        if(err){
            res.status(500).send({message:"Error-No se pudo agregar la cancion"});
        }else{
            if(!songUpdated){
        ...
        res.status(200).send({message:"No ha subido cancion"});
    }
}

function getSongFile(req,res) {
    var song_file = req.params.songFile;
    var path_file='./uploads/albums/'+song_file;
